...
public class ApiRO<T> {
     ...
     * <p><strong>直接返回为没有具体提示词的失败响应</strong></p>
     *
     * @param <T> 响应体的类型
     * @return 返回一个空的 ApiRO 对象
     */
    public static <T> ApiRO<T> unknown() {
        ApiRO<T> apiRO = new ApiRO<>();
        apiRO.status = 0;
        apiRO.msg = "fail";
        return apiRO;
    }
     ...
     */
    public static <T> ApiRO<T> fail(String msg) {
        return new ApiRO<T>().msg(msg).status(0);
    }
